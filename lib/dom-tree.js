      ...
      switch(node.type) {
        ...
        case TAG_CLOSE:
          ...
          if(stack.length && tagName != stack[stack.length-1]) {
            ...
            } else if(!config || config.createOrphanTags) {
              node.type = TAG_OPEN;
              if(!cursor.children) {cursor.children = [node];}
              else {cursor.children.push(node);}
            }
...
//  var output = parser.parse('<html></p><p>B<i></i></i></i></p></html>');
//  var output = parser.parse('<html><br foo></br></html>');
    var output = parser.parse('<html><p>xxx<p>yyy</p></p></html>');
//  var output = parser.parse(fs.readFileSync('/tmp/foo/htmlparser2/tests/index.html--').toString())
  //console.log(output);

  var r = buildDOMTree(output, basicConfig);
